...
export default function CompanyProjects() {
    ...
    if (projects.length === 0) {
        return (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center animate-in fade-in duration-300">
                ...
                <h2 className="text-xl font-bold text-gray-700">Nessun Progetto Registrato</h2>
                <p className="text-gray-500 mt-2 font-medium">Non è ancora stato registrato alcun progetto per la tua azienda.</p>
            </div>
        );
    }
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 mt-6 md:grid-cols-2 gap-6 animate-in fade-in duration-300">
                            ...
                            >
                                <CardHeader className="flex justify-between items-start pt-5 px-6">
                                    <div className="flex flex-col gap-1 w-2/3">
                                        ...
                                        <div className="flex items-center text-sm text-gray-500 font-medium truncate" title={project.collaborators?.join(", ")}>
                                            <Users size={14} className="mr-1 min-w-[14px]" />
                                            {project.collaborators && project.collaborators.length > 0
                                                ? project.collaborators.join(", ")
                                                : "Nessun freelancer assegnato"}
                                        </div>
                                    </div>
                                ...
                                {selectedProject && (
                                    <div className="flex flex-col gap-6">
                                        ...
                                        <div className="bg-gray-50 p-4 rounded-lg border border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Costo Totale Stimato</p>
                                                <p className="text-xl font-bold text-purple-700">
                                                    {selectedProject.budget && selectedProject.budget > 0
                                                        ? `${selectedProject.budget.toLocaleString('it-IT')} ${selectedProject.currency}`
                                                        : "Da definire"}
                                                </p>
                                            </div>
                                            <div className="flex flex-col gap-2">
                                                {selectedProject.proposalDate && <div className="flex justify-between border-b border-gray-200 pb-1"><p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Proposto il</p><p className="text-sm font-medium text-gray-800">{selectedProject.proposalDate}</p></div>}
                                                {selectedProject.acceptanceDate && <div className="flex justify-between border-b border-gray-200 pb-1"><p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Accettato il</p><p className="text-sm font-medium text-gray-800">{selectedProject.acceptanceDate}</p></div>}
                                                {selectedProject.completionDate && <div className="flex justify-between"><p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Concluso il</p><p className="text-sm font-medium text-gray-800">{selectedProject.completionDate}</p></div>}
                                            </div>
                                        </div>
                                        ...
                                        {selectedProject.collaborators && selectedProject.collaborators.length > 0 && (
                                            <div>
                                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Team di Freelancer Assegnati ({selectedProject.collaborators.length})</p>
                                        ...
                                        <div><p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Descrizione Completa</p><p className="text-sm text-gray-700 whitespace-pre-wrap">{selectedProject.description || "Nessuna descrizione fornita."}</p></div>
                                    </div>
                                )}
                            </Modal.Body>
                            <Modal.Footer className="border-t border-gray-100"><p className="text-sm text-gray-400 w-full text-center">Clicca fuori dalla finestra per chiudere.</p></Modal.Footer>
                        </Modal.Dialog>
                    </Modal.Container>
                </Modal.Backdrop>
            </Modal>
